...
`QSLSend` 包含 `id: number`、`callsign: string`、`sentAt: string`、`confirmedAt: string | null`、`status: 'received' | 'returned' | 'not_received' | null` 和 `trackingNumber: string | null`。`status` 为 `received` 时表示邮件已收到，为 `returned` 时表示邮件被退回，为 `not_received` 时表示经确认对方未收到但未收到退回的卡；`null` 表示状态尚未确认。
...
此输入没有 `provider` 字段，且服务端强制将 `callsign_email_cache.provider` 写为 `manual`。通过该接口写入的记录视为人工修正来源，不能在请求中指定其他来源值。

### 邮件发送输入

`SendEmailInput` 为：

- `to: string[]`，必填，至少包含 1 个非空字符串
- `cc: string[]`，可选；缺省时服务端按空数组处理
- `bcc: string[]`，可选；缺省时服务端按空数组处理
- `subject: string`，必填，非空
- `body: string`，必填，非空
- `contentType: 'text' | 'html'`，可选，默认 `text`

该对象禁止额外字段。服务端会对每个收件地址执行 `trim()`；若出现空字符串地址则请求失败。
...
确认发件时，`id` 必须是正整数，`status` 必须为 `received`、`returned` 或 `not_received`。
...
`<>` 包裹的呼号（如 `<SELF>`）命中 `config/values.ts` 中 `selfInfo.emailAddresses` 配置时，两个方向的响应 `comeFrom` 均为 `config`、`realtime` 均为 `true`，且不含 `lastUpdate`；该配置来源不写入 `callsign_email_cache`。
