...
export const Callsign2EmailRouter = new Router({
  ...
  onRootMatch: async (req, _res, ctx) => {
    ...
    if (isWrappedCallsign(normalizedCallsign)) {
      const configuredEmail = selectConfiguredEmailByCallsign(normalizedCallsign);
      ...
      const wrappedCache = await selectLatestCallsignEmailCache(normalizedCallsign);
      ctx.data =
        wrappedCache !== null
          ? toCachedResponse(mode, wrappedCache)
          : { callsign: normalizedCallsign, email: null, realtime: false, mode };
      return;
    }
