...
export const SelectCommunicationLogByIdRouter = new Router({
  matcher: (pathname) => /^\/communication-log\/id\/\d+$/.test(pathname),
  name: 'communication-log-by-id',
  onRootMatch: async (request, _response, ctx) => {
    const id = Number(request.ourl.pathname.match(/^\/api\/select\/communication-log\/id\/(\d+)$/)?.[1]);
    ctx.data = await selectCommunicationLogById(id);
  },
});
